...
function isMailerConfigured() {
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
        console.warn("Contact email not sent: EMAIL_USER and/or EMAIL_PASSWORD environment variables are missing.");
        return false;
    }
    if (!process.env.EMAIL_TO && !process.env.EMAIL_USER) {
        console.warn("Contact email not sent: EMAIL_TO is missing and EMAIL_USER cannot be used as a fallback.");
        return false;
    }
    return true;
}
...
function createTextBody({ name, email, message, phone }) {
    return `You have received a new message from your portfolio website.\n\nName: ${name}\nEmail: ${email}\n${phone ? `Phone: ${phone}\n` : ""}Message:\n${message}\n`;
}
function createHtmlBody({ name, email, message, phone }) {
    return `
